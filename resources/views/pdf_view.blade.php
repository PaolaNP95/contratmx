...
<body>
    <div class="container m-5">
        <div class="row">
            <img src="{{ asset('img/logo_completo.png') }}" class="w-25" alt="" loading="lazy">
        </div>
    ...
    <div class="container">
        ...
        <p class="text-justify" style="text-indent: 30px">
            ...
            para darle a conocer la cotización que con anterioridad solicitó con respecto a los siguientes servicios: 
    ...
    <div class="container bg-dark text-white text-center">
          <p class="p-n3 ">COTIZACIÓN Nº {{$budget_id}}</p>
    </div>
    <div class="card m-5">
        <div class="card-body">
            <div class="row">   
                <div class="col">
                    <table class="table table-hover table-sm text-center">
                        ...
                        <tbody>
                            @foreach($product->item as $items)
                            <tr>
                                <th scope="row">{{$items->id}}</th>
                                <td><p >{{$items->name}} </p></td>
                                <td><p >${{$items->price}} </p></td>
                                <td><p >{{$items->selected->quantity}} </p></td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
